...
from PySide6.QtWidgets import QWidget, QTableWidgetItem, QPushButton, QMessageBox
...
class AmministratoreView(QWidget):
    ...
    def backupBtnClicked(self, mainPath, amministratore):
        ...
        obj.ripristinaBackup.clicked.connect(lambda : self.ripristinaBackup(mainPath, amministratore))
    ...
    def caricainfoAccountView(self, mainPath, fileName, account, amministratore, lista):
        ...
        try:
            ...
            obj.numeroTelefonicoDaIns.setText(f"{account.numeroTelefonico}")
    ...
    def resetDatabaseControl(self, mainPath, amministratore):
        ...
        obj = self.caricaView(mainPath, name)
        self.removeAndAdd(obj)
        obj.info.setText("Sei sicuro di voler resettare \nl'intero Database?")
    ...
    # Metodo che gestische l'opzione di scelta dell'utente per il ripristino del backup
    def ripristinaBackup(self, mainPath, amministratore):
        name = "yesNoWindow.ui"
        obj = self.caricaView(mainPath, name)
        self.removeAndAdd(obj)
        obj.info.setText("Sei sicuro di voler ripristinare \nl'ultimo backup disponibile?")
        obj.yesBtn.clicked.connect(lambda: self.ripristinaBackupYesClicked(mainPath, amministratore))
        obj.noBtn.clicked.connect(lambda: self.backupBtnClicked(mainPath, amministratore))

    # Metodo che richiama il Controller per ripristinare il backup
    def ripristinaBackupYesClicked(self, mainPath, amministratore):
        Controller().ripristinaBackup()
        self.backupBtnClicked(mainPath, amministratore)
